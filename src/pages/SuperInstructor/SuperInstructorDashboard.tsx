...
const SuperInstructorDashboard: React.FC = () => {
    ...
    if (!className) return (
        <div className="flex items-center justify-center min-h-[60vh]">
            <div className="bg-primary/10 border-2 border-primary/20 rounded-[3rem] p-20 text-center max-w-2xl shadow-2xl backdrop-blur-3xl animate-fadeIn">
                ...
                <h3 className="text-3xl font-black text-foreground mb-4 italic tracking-tight uppercase">Access Restricted</h3>
                ...
                return (
                    <div className="premium-card overflow-hidden animate-fadeIn">
                        <div className="p-6 md:p-10 border-b border-surface-border flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-surface-light/30">
                            <h3 className="text-xl md:text-2xl font-black text-foreground italic tracking-tight">Pending Instructor <span className="text-primary italic">Approvals</span></h3>
                            ...
                            <>
                                ...
                                <div className="md:hidden space-y-4 p-4">
                                    {pendingInstructors.map(inst => (
                                        <div key={inst.id} className="bg-surface-dark/50 p-6 rounded-[1.5rem] border border-surface-border shadow-lg">
                                            <div className="mb-4">
                                                <h4 className="font-black text-foreground italic text-lg">{inst.name}</h4>
                                            ...
                                            <div className="flex justify-between items-center mb-6 pt-4 border-t border-surface-border/50">
                                                ...
                                                <span className="text-xs text-foreground italic font-medium">{inst.phone || 'N/A'}</span>
                                            </div>
                                            <button
                                ...
                                <div className="hidden md:block overflow-x-auto">
                                    <table className="w-full text-left text-sm text-accent-gray">
                                        <thead className="bg-surface-dark text-foreground font-black uppercase text-[10px] tracking-[0.2em]">
                                            <tr>
                                        ...
                                        <tbody className="divide-y divide-surface-border">
                                            {pendingInstructors.map(inst => (
                                                <tr key={inst.id} className="hover:bg-primary/5 transition-colors group">
                                                    <td className="p-8 font-black text-foreground italic text-lg">{inst.name}</td>
    ...
    return (
        <div className="space-y-8 min-h-screen p-4 md:p-12 transition-colors duration-500">
            <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-8 mb-16">
                <div>
                    <h2 className="text-5xl font-black text-foreground tracking-tighter italic">Super Instructor <span className="text-gradient-red italic">Dashboard</span></h2>
            ...
            <div className="flex flex-wrap gap-4 border-b border-surface-border pb-1 mb-16 overflow-x-auto no-scrollbar scroll-smooth">
                {[
                    ...
                    >
                        ...
                        {tab.id === 'pending' && pendingInstructors.length > 0 && <span className="bg-primary text-white text-[10px] px-2.5 py-1 rounded-full font-black animate-bounce ml-3 shadow-lg">{pendingInstructors.length}</span>}
                    </button>
                ))}
            </div>
