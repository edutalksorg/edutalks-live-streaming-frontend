...
const StudentDashboard: React.FC = () => {
    ...
    return (
        <div className="space-y-12 pb-20 max-w-7xl mx-auto px-4 md:px-0 relative transition-colors duration-500">
            ...
            <div className="relative overflow-hidden bg-surface rounded-[2rem] md:rounded-[3.5rem] p-8 md:p-20 text-foreground shadow-premium border border-surface-border group transition-all duration-500">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-12">
                        ...
                        <div className="flex flex-row lg:flex-col gap-4 md:gap-6 overflow-x-auto pb-4 lg:pb-0 lg:overflow-visible no-scrollbar">
                            ...
                            <div onClick={(e) => checkAccess(e, () => navigate('/student/materials'))} className="flex-shrink-0 bg-surface-light/50 backdrop-blur-3xl border border-surface-border p-4 sm:p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] text-center w-32 sm:w-40 md:w-64 hover:border-primary/30 transition-all cursor-pointer group/card lg:translate-x-10 shadow-premium block">
                                <div className="text-3xl sm:text-4xl md:text-7xl font-black text-foreground italic tracking-tighter group-hover/card:scale-110 transition-transform">{dashboardData?.stats.studyMaterials}</div>
                        ...
                        <div className="bg-surface border border-primary/30 p-6 rounded-[2rem] shadow-[0_10px_40px_-10px_rgba(238,29,35,0.4)] relative overflow-hidden">
                            ...
                            <div className="flex items-start justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 className="text-lg font-black text-foreground italic tracking-tight leading-none">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-16 pt-10">
                            ...
                            >
                                ...
                                >
                                    <div className="flex items-center gap-4 md:gap-10">
                                        <div className={`w-16 h-16 md:w-24 md:h-24 rounded-2xl md:rounded-[2.5rem] flex items-center justify-center transition-all duration-700 ${expandedSubject === subject.id ? 'bg-primary text-white rotate-12 shadow-2xl shadow-primary/40' : 'bg-surface-dark text-foreground border-2 border-surface-border group-hover/header:bg-primary group-hover/header:rotate-12 shadow-xl'
                                            }`}>
                                            <FaBook size={24} className="md:size-[36px]" />
                                        </div>
                                        <div className="space-y-1 md:space-y-2">
                                            <h3 className="text-xl md:text-4xl font-black text-foreground tracking-tight italic group-hover/header:text-primary transition-colors">{subject.name}</h3>
                                            <div className="flex items-center gap-2 md:gap-4 text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] text-accent-gray flex-wrap">
                                                ...
                                                {subject.instructor_name && (
                                                    <>
                                                        ...
                                                        <span className="text-foreground/70 italic block sm:inline">By {subject.instructor_name}</span>
                                                    </>
                                                )}
                                            </div>
                                        </div>
                                ...
                                {expandedSubject === subject.id && (
                                    <div className="px-6 md:px-12 pb-8 md:pb-12 pt-4 space-y-8 md:space-y-12 animate-in fade-in slide-in-from-top-10 duration-1000">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                                                            ...
                                                            <div className="bg-surface-dark p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-surface-border flex flex-col gap-6 group/exam hover:bg-surface-light transition-all duration-500 h-full">
                                                                <div className="flex items-start gap-4 md:gap-6">
                                                                    <div className={`w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl flex items-center justify-center shadow-lg transition-transform group-hover/exam:rotate-12 flex-shrink-0 ${exam.status === 'Completed' ? 'bg-accent-emerald text-white shadow-accent-emerald/20' :
                                                                        exam.status === 'Pending' ? 'bg-surface text-foreground border border-surface-border' :
                                                                    ...
                                                                    <div className="min-w-0 flex-1">
                                                                        <div className="text-base md:text-xl font-black text-foreground tracking-tight leading-tight mb-2">{exam.title}</div>
                                            ...
                                            {/* Materials */}
                                            <div className="space-y-6 md:space-y-8">
                                                <h4 className="text-[10px] md:text-[12px] font-black text-foreground uppercase tracking-[0.4em] mb-6 md:mb-10 italic">Premium Materials</h4>
                                                <div className="space-y-4 md:space-y-5">
                                                    {subject.notes.map((note, idx) => (
                                                        <div key={idx} className="bg-surface-dark p-4 md:p-6 rounded-[1.5rem] md:rounded-[2.5rem] border border-surface-border flex items-center justify-between group/note hover:bg-surface-light transition-all duration-500">
                                                            <div className="flex items-center gap-4 md:gap-6">
                                                                <div className="w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-surface text-foreground flex items-center justify-center shadow-xl group-hover/note:bg-primary group-hover/note:text-white transition-all border border-surface-border">
                                                                    <FaFileAlt size={16} className="md:size-[20px]" />
                                                                </div>
                                                                <div>
                                                                    <div className="text-sm md:text-lg font-black text-foreground tracking-tight group-hover/note:text-primary transition-colors">{note.title}</div>
                                                                    <div className="text-[8px] md:text-[10px] text-accent-gray font-black uppercase tracking-widest mt-1">Resource</div>
                                                                </div>
                                                            </div>
                                                            <div onClick={(e) => checkAccess(e, () => window.open(`${import.meta.env.VITE_API_URL.replace('/api', '')}${note.file_path}`, '_blank'))} className="cursor-pointer bg-surface border border-surface-border text-foreground px-4 md:px-6 py-2 md:py-3 rounded-lg md:rounded-xl font-black text-[8px] md:text-[10px] tracking-widest hover:bg-primary hover:text-white transition-all shadow-sm">
                                                                PDF
                                                            </div>
                                                        </div>
                                                    ))}
                ...
                <div className="xl:col-span-4 space-y-16">
                    ...
                    <div className="bg-surface rounded-[2rem] md:rounded-[3.5rem] shadow-premium border border-surface-border p-8 md:p-12 space-y-8 md:space-y-10 relative overflow-hidden group">
                        ...
                        <h2 className="text-2xl md:text-3xl font-black text-foreground flex items-center gap-4 italic tracking-tight">
                                    ...
                                    <div key={i} className="flex flex-col gap-3 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2.5rem] bg-surface-dark border border-surface-border hover:border-emerald-500/20 transition-all duration-500 group/result">
                                        <div className="flex justify-between items-start">
                                            <div className="min-w-0">
                                                <h5 className="font-black text-lg text-foreground truncate tracking-tight mb-1">{result.title}</h5>
                                        ...
                                        {result.review_text && (
                                            <div className="mt-3 text-[9px] text-foreground italic opacity-70 border-l-2 border-emerald-500/30 pl-3 leading-relaxed">
                                                "{result.review_text}"
                                            </div>
                                        )}
                                    </div>
